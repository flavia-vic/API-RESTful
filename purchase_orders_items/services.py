...
class PurchaseOrdersItemsService:
	...
	def find_by_id_purchase_order_id(self,purchase_order_id):	
		purchase_order = PurchaseOrderModel.find_by_id(purchase_order_id)
		if purchase_order:
		    purchase_orders_items = PurchaseOrdersItemsModel.find_by_id_purchase_order_id(purchase_order_id)
	...
	def create(self,**kwargs):
		...
		return jsonify({'message':'Purchase Order {} não encontrado'.format(kwargs['purchase_order_id'])})
